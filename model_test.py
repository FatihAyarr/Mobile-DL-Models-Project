# with this script a model can only be tested for one class
# lines with description: 20, 71, 96, 126, 139, 142
...
base_path = "../test_photos" # path of the photo folder to test
data = []
...
# function previously used to read photos. not used in this working script.
'''
...
def calculateAccuracy(total, i, current): 
    i_ = i
    if current != "class1" : # if class1 photos are being tested, write class1, if class2, write class2
        i_ -= 1
...
TFLITE_MODEL = '/FatihNewModel/TFLite_Model_Maker/model756.tflite' # path of the model to be tested
...
for dataChunk in pbar:
    ...
    for i in dataChunk:
        ...
        img = imread(i)
        img_resized = resize(img,(224,224,3)) # resizes according to input size
    ...
    result['link'].append("https://xxxxx.s3.amazonaws.com/yyyyy/" + data_id + ".jpg")
...
#tf_result.to_csv('results_class2.csv')
#tf_result.to_excel('/fatihmodel_class2_results.xlsx')
print('total time: '+ str((end-start)*1000) + 'sn')
